...
double get_us_snpe(struct timeval t) { return (t.tv_sec * 1000000 + t.tv_usec); }
...
class PredictorSnpe {
  public:
    PredictorSnpe(const string &model_file, int batch, int mode, bool verbose, bool profile);
    void PredictSnpe(int* inputData_quantize, float* inputData_float, bool quantize);
...
PredictorSnpe::PredictorSnpe(const string &model_file, int batch, int mode, bool verbose, bool profile) {
  ...
  if(verbose_) {
    LOG(INFO) << "Model loading (C++): " << (get_us_snpe(stop_time) - get_us_snpe(start_time))/1000 << "ms \n";
  }
  
}

void PredictorSnpe::PredictSnpe(int* inputData_quantize, float* inputData_float, bool quantize) {
  ...
  // log model inference
  if(verbose_) {
    LOG(INFO) << "Model computation (C++): " << (get_us_snpe(stop_time) - get_us_snpe(start_time))/1000 << "ms \n"; 
  }
...
PredictorContext NewSnpe(char *model_file, int batch, int mode, bool verbose, bool profile) {
  try {
    const auto ctx = new PredictorSnpe(model_file, batch, mode, verbose, profile);
    return (void *) ctx;
...
void PredictSnpe(PredictorContext pred, int* inputData_quantize, float* inputData_float, bool quantize) {
  auto predictor = (PredictorSnpe *)pred;
  if (predictor == nullptr) {
    return;
  }
  predictor->PredictSnpe(inputData_quantize, inputData_float, quantize);
  return;
}
