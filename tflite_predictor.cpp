...
double get_us_tflite(struct timeval t) { return (t.tv_sec * 1000000 + t.tv_usec); }
...
*/
class PredictorTflite {
  public:
    PredictorTflite(const string &model_file, int batch, int mode, bool verbose, bool profile);
    void PredictTflite(int* inputData_quantize, float* inputData_float, bool quantize);
...
PredictorTflite::PredictorTflite(const string &model_file, int batch, int mode, bool verbose, bool profile) {
  ...
  if(verbose_) {
    LOG(INFO) << "Model loading (C++): " << (get_us_tflite(stop_time) - get_us_tflite(start_time))/1000 << "ms \n";
  }
  mode_ = mode;
  batch_ = batch;
...
void PredictorTflite::PredictTflite(int* inputData_quantize, float* inputData_float, bool quantize) {
  ...
  // log model inference
  if(verbose_) {
    LOG(INFO) << "Model computation (C++): " << (get_us_tflite(stop_time) - get_us_tflite(start_time))/1000 << "ms \n"; 
  }
...
PredictorContext NewTflite(char *model_file, int batch, int mode, bool verbose, bool profile) {
  try {
    const auto ctx = new PredictorTflite(model_file, batch, mode, verbose, profile);
    return (void *) ctx;
...
void PredictTflite(PredictorContext pred, int* inputData_quantize, float* inputData_float, bool quantize) {
  auto predictor = (PredictorTflite *)pred;
  if (predictor == nullptr) {
    return;
  }
  predictor->PredictTflite(inputData_quantize, inputData_float, quantize);
  return;
}
